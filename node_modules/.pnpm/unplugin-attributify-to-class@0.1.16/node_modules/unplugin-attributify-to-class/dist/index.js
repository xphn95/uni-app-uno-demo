        ...
        valuedAttributes.forEach(([sourceStr, name, _, content]) => {
          ...
          if (["class", "className"].includes(name)) {
            ...
            if (prefixedOnly && !name.startsWith(prefix))
              return;
            const attributifyToClass = content.split(splitterRE).filter(Boolean).map((v) => v === "~" ? `${classPrefix}${_name}` : `${classPrefix}${_name}-${transformEscape ? (0, import_utils.transformSelector)(v, transformRules) : v}`).join(" ");
