...
var extractorAttributify = (options) => {
  ...
  const transformRules = (options == null ? void 0 : options.transformRules) ?? import_utils.defaultRules;
        ...
        valuedAttributes.forEach(([sourceStr, name, _, content]) => {
          ...
          if (!content) {
            ...
            if (isValidSelector(name) && nonValuedAttribute !== false) {
              if (!ignoreNonValuedAttributes.includes(name))
                option.selectors.push(transformEscape ? (0, import_utils.transformSelector)(`${classPrefix}${name}`, transformRules) : `${classPrefix}${name}`);
            }
            return;
          }
          ...
          if (["class", "className"].includes(name)) {
            ...
            if (prefixedOnly && !name.startsWith(prefix))
              return;
            const attributifyToClass2 = content.split(splitterRE).filter(Boolean).map((v) => v === "~" ? `${classPrefix}${_name}` : `${classPrefix}${_name}-${transformEscape ? (0, import_utils.transformSelector)(v, transformRules) : v}`).join(" ");
