...
const colors = require('./unocss-preset-weapp.80bd545b.cjs');
...
const animations = [
  ...
  [/^animate-(.+)$/, ([, name], { theme }) => {
    name = utils.restoreSelector(name, theme.transformRules);
    const kf = theme.animation?.keyframes?.[name];
    if (kf) {
  ...
  [/^animate-ease(?:-(.+))?$/, ([, d], { theme }) => ({ "animation-timing-function": theme.easing?.[d || "DEFAULT"] ?? colors.handler.bracket.cssvar(d) }), { autocomplete: "animate-delay-$easing" }],
  [
...
const backgroundStyles = [
  [/^bg-(.+)$/, ([, d], { theme }) => {
    d = utils.restoreSelector(d, theme?.transformRules);
    if (bgUrlRE.test(d))
    ...
    if (bgLengthRE.test(d) && colors.handler.bracketOfLength(d) != null)
      return { "background-size": colors.handler.bracketOfLength(d).split(" ").map((e) => colors.handler.fraction.auto.px.cssvar(e)).join(" ") };
    if (bgPositionRE.test(d) && colors.handler.bracketOfPosition(d) != null)
      return { "background-position": colors.handler.bracketOfPosition(d).split(" ").map((e) => colors.handler.position.fraction.auto.px.cssvar(e)).join(" ") };
  }],
  ...
  [/^(?:bg-gradient-)?from-op(?:acity)?-?(.+)$/, ([, opacity]) => ({ "--un-from-opacity": colors.handler.bracket.percent(opacity) })],
  [/^(?:bg-gradient-)?via-op(?:acity)?-?(.+)$/, ([, opacity]) => ({ "--un-via-opacity": colors.handler.bracket.percent(opacity) })],
  [/^(?:bg-gradient-)?to-op(?:acity)?-?(.+)$/, ([, opacity]) => ({ "--un-to-opacity": colors.handler.bracket.percent(opacity) })],
...
function handlerRounded([, a = "", s], { theme }) {
  s = utils.restoreSelector(s, theme?.transformRules);
...
const fonts = [
  ...
  [/^text-size-(.+)$/, ([, s], { theme }) => {
    const themed = core.toArray(theme.fontSize?.[s]);
    const size = themed?.[0] ?? colors.handler.bracket.cssvar.global.rpx(s);
    if (size != null)
  ...
  [
    /^(?:font-)?(?:leading|lh)-(.+)$/,
    ([, s], { theme }) => ({ "line-height": theme.lineHeight?.[s] || colors.handler.bracket.cssvar.global.remToRpx(s) }),
...
function handleInsetValue(v, { theme }) {
  v = utils.restoreSelector(v, theme?.transformRules);
  return theme.spacing?.[v] ?? colors.handler.bracket.cssvar.global.auto.fraction.rpx(v);
}
...
const objectPositions = [
  ...
  [/^object-(.+)$/, ([, d], { theme }) => {
    ...
    if (colors.handler.bracketOfPosition(d) != null)
      return { "object-position": colors.handler.bracketOfPosition(d).split(" ").map((e) => colors.handler.position.fraction.auto.px.cssvar(e)).join(" ") };
...
function handleTranslate([, d, b], { theme }) {
  b = utils.restoreSelector(b, theme?.transformRules);
